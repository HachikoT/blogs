- [kubeadm init](#kubeadm-init)
	...
	- [mark-control-plane](#mark-control-plane)
	- [bootstrap-token](#bootstrap-token)
	- [kubelet-finalize](#kubelet-finalize)
	- [addon](#addon)
	- [show-join-command](#show-join-command)
- [kubeadm join](#kubeadm-join)
	- [control-plane-prepare](#control-plane-prepare)
	- [check-etcd](#check-etcd)
	- [kubelet-start](#kubelet-start-1)
- [参考资料](#参考资料)
...
clusterName: "kubernetes"                                # 集群名称（kubeconfig文件会记录）
...
```go
// 生成`kube-apiserver`，`kube-controller-manager`，`kube-scheduler`静态pod yaml
...
func WriteKubeletDynamicEnvFile(cfg *kubeadmapi.ClusterConfiguration, nodeReg *kubeadmapi.NodeRegistrationOptions, registerTaintsUsingFlags bool, kubeletDir string) error {
	// --container-runtime-endpoint=${nodeRegistration.criSocket}
	// --pod-infra-container-image=${imageRepository}/pause:3.10
	// --hostname-override=${nodeRegistration.name}
...
```go
// 生成`/var/lib/kubelet/config.yaml`，kubelet完整的配置文件
...
1. 将ClusterConfiguration配置上传为`kubeadm-config` configmap
2. 将KubeletConfiguration配置上传为`kubelet-config` configmap

## upload-certs

默认是不会上传证书的，需要主动设置`--upload-certs`选项。

将所有ca证书和密钥，以及sa证书和私钥配置上传为`kubeadm-certs` secret，数据都使用`${certificateKey}`密钥加密，如果没有设置加密密钥那么会使用临时生成的密钥加密。

## mark-control-plane

给节点打污点，`${nodeRegistration.taints}`

## bootstrap-token

1. 上传bootstraptoken为secret，存活时间默认24h
2. 创建`cluster-info` configmap

## kubelet-finalize

修改`kubelet.conf`文件，使其使用kubelet自动轮转的证书密钥`/var/lib/kubelet/pki/kubelet-client-current.pem`
...
control-plane-prepare        准备用作控制平面的机器
  /download-certs              下载ca相关证书密钥
  /certs                       根据ca生成缺失的证书密钥
  /kubeconfig                  根据ca生成缺失的kubeconfig文件
  /control-plane               生成控制平面静态pod yaml
check-etcd                   检查etcd集群是否正常
...
controlPlane:                                            # 作为控制平面节点加入的配置（需要设置--control-plane参数才会有默认值）
  localAPIEndpoint:
    advertiseAddress: ""
    bindPort: 6443
  certificateKey: ""                                     # 用来解压从secret中下载的ca相关证书密钥，如果没有设置，那么直接使用本地的ca证书
skipPhases: []
# patches:
#   directory:
timeouts:
...
1. 利用bootstraptoken生成k8s客户端下载`kubeadm-certs` secret中下载ca证书（需要设置`${certificateKey}`解压密钥，没有设置就使用本地的ca证书密钥）
2. 根据ca生成缺失的证书密钥
3. 根据ca生成缺失的kubeconfig文件
4. 生成控制平面静态pod yaml

## check-etcd

查询所有etcd pod，连接etcd集群，检查集群是否处于健康状态
